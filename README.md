...
This project was built to demonstrate how easy it can be to build your own scripting language with ANTLR4.
It includes a basic language with support for variables and control structures, as well as a more advanced version with first-class functions,
...
This project contains two implementations of a simple scripting language, _nx-script-basic_ and _nx-script_.
The former is a very basic language, intended to show the basics of ANTLR4 and interpreters in general. The latter is a more advanced version of the language, with more features and capabilities.
This is the one that is "actively" developed.

_nx-script-basic_:

- basic types (Bool, Number, String)
...
- all of the above
- collection types (`Seq`, `Obj`)
- subscrupting & member access (`foo.bar` and `foo[index]`)
- first-class functions (functions are values) + lambda (anonymous) functions
- vscode syntax-support for `.nx` files

> [!TIP]
> We've got a list of all the features we want to implement in the future in the `ROADMAP.md` file.
> Feel free to contribute! Either by implementing a feature or by suggesting one.
